...
## 14. Test Metrics & Health (Observability)

Vì các microservice trong YAS được cấu hình cổng quản lý riêng biệt là `8090` (`management.server.port: 8090`), bạn cần gọi đúng cổng `8090` để kiểm tra metrics/health.

### Bước 1: Tạo pod curl (nếu chưa có hoặc đã bị Completed)
Nếu pod `curl-as-cart` đã chạy xong và ở trạng thái `Completed`, xóa và tạo lại:
```bash
kubectl delete pod curl-as-cart -n yas-dev --ignore-not-found
...
### Bước 2: Test API metrics bằng curl
Gọi endpoint metrics của `product-service` qua cổng `8090`:
```bash
kubectl exec -n yas-dev curl-as-cart -c curl -- \
  curl -s http://product.yas-dev.svc.cluster.local:8090/actuator/prometheus | head
```

Kết quả mong muốn:
Trả về các thông tin metric của JVM/Micrometer dạng:
```text
